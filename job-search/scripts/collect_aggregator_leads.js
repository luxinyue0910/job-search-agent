...
function inferCompanyAndRole(text) {
  const lines = String(text || "")
    ...
    .filter(Boolean)
    .filter((line) => !/^(apply|save|new|easy apply|view job|posted|promoted)$/i.test(line));
  ...
  let company = "";
  if (roleIndex > 0) company = lines[roleIndex - 1];
  if (!company && roleIndex >= 0 && lines[roleIndex + 1]) company = lines[roleIndex + 1];
  if (!company) company = lines.find((line) => /^[A-Z][A-Za-z0-9 .,&-]{1,60}$/.test(line)) || "";
  return {
...
async function extractVisibleLeads(page, provider, sourceUrl) {
  const raw = await page.evaluate(() => {
    ...
    const containers = Array.from(document.querySelectorAll("article, li, [data-testid*='job'], [class*='job'], [class*='Job'], div"));
    return containers
      .filter(isVisible)
      .map((node) => {
